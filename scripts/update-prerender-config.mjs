...
try {
  ...
  const prerenderConfig = angularJson.projects['personal-blog-front'].architect.prerender;
  if (prerenderConfig) {
    prerenderConfig.options.routes = PRERENDER_ROUTES;
    
    // Write updated angular.json
    fs.writeFileSync(angularJsonPath, JSON.stringify(angularJson, null, 2));
    console.log(`Updated prerender configuration with ${PRERENDER_ROUTES.length} routes`);
  } else {

...
export class NewsletterConfirmationComponent implements OnInit {
  ...
  private confirmSubscription(newslettersId: string): void {
    this.newsletterService.confirmSubscription(newslettersId).subscribe({
      next: (response: ConfirmSubscriptionEnum) => {
  ...
  private handleConfirmationResponse(response: ConfirmSubscriptionEnum): void {
    switch (response) {
      case ConfirmSubscriptionEnum.SUBSCRIPTION_CONFIRMED:
        this.isSuccess = true;
        this.message =
